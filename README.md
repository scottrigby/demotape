...
```jsonc
{
  ...
  "features": {
    "ghcr.io/scottrigby/demotape/demotape:0.9.3": {}
  }
}
```
...
|---|---|---|
| `version` | `main` | Git ref of `scottrigby/demotape` to install — branch (`main`), tag (`v0.9.3`), or commit. Pin to a tag for reproducible builds. |
...
git commit -am "Bump to v0.9.3"
git push origin main                    # CI tags v0.9.3 + publishes OCI feature
```
