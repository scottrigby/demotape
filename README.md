...
```jsonc
{
  ...
  "features": {
    "ghcr.io/scottrigby/showtape/showtape:0.8.0": {}
  }
}
```
...
|---|---|---|
| `version` | `main` | Git ref of `scottrigby/showtape` to install — branch (`main`), tag (`v0.8.0`), or commit. Pin to a tag for reproducible builds. |
...
git commit -am "Bump to v0.8.0"
git push origin main                    # CI tags v0.8.0 + publishes OCI feature
```
