...
- **Terminal sessions are multi-rendered, one VHS run per unique pane size.** A session that appears at two viewport sizes (e.g. `960×540` in split-screen steps and `1920×1080` in a full-screen step) gets two independent VHS renders — same shell, same commands, same scrollback content, but different line-wrap at the viewport boundary. Per-step slices route to the matching-dim MP4. The output resolution caps the unique dim count at four: `1920×1080`, `960×1080`, `1920×540`, `960×540` — so worst case is four VHS runs per session, typically one or two. Cheap (~10–30 s per render) and deterministic. The alternative (rescaling at composite time) was rejected because it produces wrong line wrap relative to the new viewport.
