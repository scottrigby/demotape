...
- **Session terminal geometry is auto-computed from the smallest pane the session ever appears in.** All session recordings use a fixed 28px font (matching plain terminal panes so the visual weight is consistent). The tmux session cols×rows are derived from that font at the *smallest* viewport dim the session appears at across all steps — `cols = floor(inner_width / (28px × 0.60))`, `rows = floor(inner_height / (28px × 1.25))` where `inner = viewport − 2×padding`. Using the smallest dim ensures every viewport can display the full terminal grid. Steps where the session appears in a *larger* viewport (e.g., full-screen after split-screen) show the same content with empty space to the right and bottom — the scrollback looks identical across all steps rather than jumping to a different font size.
