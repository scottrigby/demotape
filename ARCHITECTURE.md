...
2. **Step duration** — `max(narration_ms, …per-pane action estimates) + pause_ms`. Each pane's estimate is a sum of per-action constants (`goto`=3 s, `fill`=0.3 s, etc.) plus a 0.5 s safety margin. Data-driven instead of narration-only, so a long form-fill flow naturally extends the step.
...
- **Terminal sessions use a tmux server + parallel VHS clients (one per unique dim).** A session that appears at two viewport sizes (e.g. `960×540` in split-screen steps and `1920×1080` in a full-screen step) starts one detached tmux server-side session, then spawns one VHS client per dim — all attaching simultaneously. The orchestrator drives commands once via `tmux send-keys` while all clients record in real time. Commands execute exactly once — safe for write-ops (`helm upgrade`, `kubectl apply`, `git push`). Font size is computed per dim so 80 columns fills each viewport. Session rows = smallest natural row count across all dims (typically identical for 16:9 dims). Per-step slices still route to the matching-dim MP4 via the same `_slice_session_video` path.
