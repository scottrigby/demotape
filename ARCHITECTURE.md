...
| `demos/paste.yaml` | `type:` vs `paste:` terminal action comparison. |
| `demos/terminal-sessions.yaml` | Multi-session terminal continuity across steps and viewport sizes. |
| `demos/sessions.yaml` | Cross-pane copy/paste: `capture:` / `paste_from:` across terminal and browser sessions. |
| `demos/browser-session.yaml` | Browser session object permanence — live Playwright contexts across steps. |
...
| `demos/.env` | Example `.env` for demos (committed; non-sensitive values only). |
| `demos/.env.example` | Template for users creating their own `.env` with real credentials. |
...
3. **Pane recording** (sequential, one per pane):
   - **Named-session browser panes** — The Playwright context is kept alive for the full render (started in Pass 2). Each step resizes the viewport, runs its `actions:` list, then captures screenshots at 30 fps in the main thread until `step_ms` elapses. Frames are encoded to MP4 via `ffmpeg image2pipe`. The page's full state — URL, scroll, sessionStorage, JS timers, form values — persists across steps without re-navigation.
   - **Non-session browser panes** — Fresh Playwright context per step. `record_video_dir` produces a WebM at the pane's exact dimensions. Cookies/localStorage from the previous step are restored via `storage_state`.
   ...
   - **`record: false` steps** — Pane actions run (advancing browser and terminal session state) but no clip is produced and the step is excluded from the output. `wait_ms:` provides a pure Python sleep with no panes required.
...
- **Named browser sessions keep the full Playwright context alive.** A single `sync_playwright().start()` instance is created at the start of `render()` and shared by all browser operations. Named sessions (`session:` declared) get a browser + context that stays open for the entire render — the page's URL, scroll, sessionStorage, JS timers, and form state all persist. Non-session panes launch and close a fresh browser per step. This design enables flows like "stay on a verification-code form field while a second session fetches the code from an inbox" that are impossible with fresh contexts.
- **Playwright's sync API cannot be called from background threads.** The sync API uses Python greenlets tied to the thread that called `sync_playwright().start()`. Calling `page.screenshot()` from a background thread produces `greenlet.error: cannot switch to a different thread`. Screenshot capture therefore runs sequentially in the main thread: a post-action burst after each action, then a fill loop for the remaining step duration. This gives "at rest" frames rather than continuous capture but is sufficient for demo content.
- **One Playwright instance per render, not per pane.** A second `sync_playwright().start()` call in the same process raises `asyncio loop already running`. The shared `_playwright` global is the solution.
- **Terminal sessions: one tmux session per id, one VHS recording per step.** A session that appears across multiple steps (and at different viewport dims) is backed by a single detached tmux server-side session that stays alive for the entire render. Each step that includes a session pane attaches a fresh VHS client to that tmux session, drives the step's actions via `tmux send-keys`, and exits when the step duration elapses. No batch recording, no time-based slicing, no scale factors — each step's MP4 is exactly what happened during that step. Commands execute exactly once (safe for write-ops: `helm upgrade`, `kubectl apply`, `git push`). `/bin/bash` with `LC_ALL=C` is used so demo terminals are clean and predictable regardless of the host's default shell.
- **Session terminal geometry auto-adapts per step via `window-size latest`.** All session recordings use a fixed 28px font. The tmux session is initialised at the smallest dim's natural grid (`cols = floor(inner_width / (28px × 0.60))`, `rows = floor(inner_height / (28px × 1.25))` where `inner = viewport − 2×padding`). The session uses `window-size latest`, so when each step's VHS client attaches, tmux automatically resizes the window to that client's natural grid — the shell receives SIGWINCH and sees the correct column count for that step's viewport. Split-screen steps format output at ~53 cols; full-screen steps at ~110 cols. No fill dots (window always matches the client). Hard-newlined scrollback from narrower steps shows with empty space on the right in wider steps — not reflowed, as expected from a real terminal resize.
- **Environment variable substitution is post-parse, string-values only.** `_load_dotenv()` reads a `.env` file from the YAML's directory into `os.environ` (with `setdefault` so the shell always wins), then `_substitute_env_vars()` walks the parsed YAML tree and replaces `${VAR}` and `${VAR:-default}` in every string value. Doing this after `yaml.safe_load()` means YAML structure is never affected by substituted content — no risk of a password containing `:` breaking YAML parsing. Required variables raise a clear `ValueError` before any rendering starts; defaults are used silently.
- **Cross-pane copy/paste via named buffers.** `capture:` (terminal or browser) and `paste_from:` (terminal or browser `fill:`) share a module-level `_session_buffers` dict that persists for the full render. Terminal `capture:` parses the tmux pane's visible output using the known `$ ` prompt format to extract the last command's output. Browser `capture:` accepts a CSS `selector:` (innerText) or `eval:` (arbitrary JS expression). Buffers flow freely between pane types and across steps.
