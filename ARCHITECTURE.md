...
- **Environment variable substitution is post-parse, string-values only.** `_load_dotenv()` reads a `.env` file from the same directory as the YAML file (sibling, not searched upward) into `os.environ` (with `setdefault` so the shell always wins), then `_substitute_env_vars()` walks the parsed YAML tree and replaces `${VAR}` and `${VAR:-default}` in every string value. Doing this after `yaml.safe_load()` means YAML structure is never affected by substituted content — no risk of a password containing `:` breaking YAML parsing. Required variables raise a clear `ValueError` before any rendering starts; defaults are used silently.
